<template>
  <div
    class="absolute left-0 top-full mt-2 z-50 bg-gray-900 border border-gray-500/20 rounded-lg shadow-xl p-3 min-w-[300px] pointer-events-none animate-scale-in"
    :class="{ 'hidden': !show }"
  >
    <!-- Arrow -->
    <div class="absolute -top-1 left-4 w-2 h-2 bg-gray-900 border-l border-t border-gray-500/20 transform rotate-45"></div>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        ...
        <span
          ...
          :class="{
            'bg-green-500/10 text-green-300': event.res?.status && event.res.status >= 200 && event.res.status < 300,
            'bg-red-500/10 text-red-300': event.res?.status && event.res.status >= 400,
            'bg-yellow-500/10 text-yellow-300': event.res?.status && event.res.status >= 300 && event.res.status < 400,
          }"
        >
          {{ event.res?.status || '—' }}
        </span>
