<template>
  <div class="bg-gray-500/5 border border-gray-500/10 rounded-lg p-4">
    ...
    <div v-else class="relative">
      ...
      <!-- Events -->
      <div class="relative flex items-center justify-between py-4 overflow-x-auto">
        <div
        ...
        >
          ...
          <!-- Hover info -->
          <div class="absolute top-6 hidden group-hover:block bg-gray-900 border border-gray-500/20 rounded px-2 py-1 text-xs text-white whitespace-nowrap shadow-lg z-20 animate-scale-in">
            <div class="font-mono">{{ event.req.method }}</div>
            <div class="text-gray-400">{{ fmtMs(event.res?.durationMs) }}</div>
          </div>
        </div>
      </div>
